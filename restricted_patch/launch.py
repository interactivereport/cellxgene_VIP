# target server/cli/launch.py
# changes made:
#       add a new click option -- @click.option("--restricted"...)
#       add "restricted" to def launch() & add the conditioning
#       also inside def launch(), add "restricted" to cli_config.update_dataset_config
...
def dataset_args(func):
    ...
    @click.option(
        "--restricted",
        default=DEFAULT_CONFIG.dataset_config.app__restricted,
        is_flag=True,
        help="Disable GetData button & disable CLI panel"
    )
    @click.option(
        "--identifier",
        default=DEFAULT_CONFIG.dataset_config.app__identifier,
        metavar="<text>",
        help="distinguish the timestamp of the instance",
    )
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        return func(*args, **kwargs)
...
def launch(
    ...
    app_config.dataset_config.dataset__app__restricted = True if restricted else False
    app_config.dataset_config.dataset__app__identifier = identifier
