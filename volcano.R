...
g <- ggplot(mtable, aes(x=logFC, y=-log10(FDR))) +
  ...
  geom_text_repel(data = subset(mtable, gene_name %in% genes), aes(label = gene_name), size = 3, box.padding = unit(0.35, "lines"), point.padding = unit(0.3, "lines"))
...
g <- g + annotate(geom="text", x=ggplot_build(g)$layout$panel_params[[1]]$x.range[1]+1, y=-log10(0.05)+yrange[2]/80, label="FDR=0.05", color="darkgreen", size=3, fontface="bold")
g <- g + annotate(geom="text", x=xrange[1]/2, y=-5, label=paste("Down-regulated:",Down), color="#5B98E6", size=4, fontface="bold")
g <- g + annotate(geom="text", x=xrange[2]/2, y=-5, label=paste("Up-regulated:",Up), color="#B41A29", size=4, fontface="bold")
g <- g + theme(panel.background = element_rect(fill = "transparent", color=NA), plot.background = element_rect(fill = "transparent", color = NA))

dpi <- 1
if(sum(strFun%in%c('png','jpeg','tiff'))>0) dpi <- 150
strImg <- gsub("csv$",strFun,strCSV)
f <- get(strFun)
f(strImg, width=6.5*dpi, height=6.5*dpi)
print(g)
a <- dev.off()
